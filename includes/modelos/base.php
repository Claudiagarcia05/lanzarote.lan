    ...
    class Base {
        ...
        function inicializar($datos = []) {
            ...
            while ($registro = $query->recuperar()) {
                ...
                $this->$campo_nuevo = $datos[$campo_nuevo] ?? null;
            }
        }
        ...
        function get_rows($consulta=[]) {
            ...
            $limit  = isset($consulta['limit'])  ? 'LIMIT '.$consulta['limit'] : '';
            $offset = isset($consulta['offset']) ? 'OFFSET '.$consulta['offset'] : '';

            $where = '';
            if (!empty($consulta['where'])) {
                foreach ($consulta['where'] as $clave => $valor) {

                    if ($valor === null) {
                        $where .= " {$clave} IS NULL AND";
                    } else {
